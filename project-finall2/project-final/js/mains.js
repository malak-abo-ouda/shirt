...
//   ScrollReveal().reveal('.textCardhead ', { origin: 'top' });
...
const
  ...
  smcolorAllEl = document.querySelectorAll(".div-color"),
...
for (let i = 0; i < smImgAllEl.length; i++) {
  smImgAllEl[i].onclick = _ => {
    ...
    clearBorder()
    smImgAllEl[i].classList.add("active-img");
    smcolorAllEl[i].classList.add("active-img");
...
function clearBorder() {
  for (let i = 0; i < smImgAllEl.length; i++) {
    smImgAllEl[i].classList.remove("active-img");
    smcolorAllEl[i].classList.remove("active-img");
